 ...
 * @author Mike Shaver
 * @see NativeJavaArray
 * @see NativeJava//package
 * @see NativeJavaClass
 */

public class NativeJavaObject implements Scriptable, Wrapper, Serializable
{
    static readonly long serialVersionUID = -6948590651130498591L;
    ...
    public NativeJavaObject(Scriptable scope, Object javaObject,
                            Class<?> staticType, bool  isAdapter)
    {
        this.parent = scope;
    ...
    public bool  hasInstance(Scriptable value) {
...
*/
    ...
    public Object getDefaultValue(Class<?> hint)
    {
        ...
        if (hint == null || hint == ScriptRuntime.StringClass) {
            ...
            if (hint == ScriptRuntime.bool Class) {
                converterName = "bool Value";
            ...
            if (converterObject is Function) {
                ...
                if (hint == ScriptRuntime.NumberClass
                    && javaObject is bool )
                {
                    bool  b = ((bool )javaObject).bool Value();
     ...
     */
    public static bool  canConvert(Object fromObj, Class<?> to) {
        int weight = getConversionWeight(fromObj, to);

        return (weight < CONVERSION_NONE);
    }

    private static readonly int JSTYPE_UNDEFINED   = 0; // undefined type
    private static readonly int JSTYPE_NULL        = 1; // null
    private static readonly int JSTYPE_bool      = 2; // bool 
    private static readonly int JSTYPE_NUMBER      = 3; // number
    private static readonly int JSTYPE_STRING      = 4; // string
    private static readonly int JSTYPE_JAVA_CLASS  = 5; // JavaClass
    private static readonly int JSTYPE_JAVA_OBJECT = 6; // JavaObject
    private static readonly int JSTYPE_JAVA_ARRAY  = 7; // JavaArray
    private static readonly int JSTYPE_OBJECT      = 8; // Scriptable

    static readonly byte CONVERSION_TRIVIAL      = 1;
    static readonly byte CONVERSION_NONTRIVIAL   = 0;
    static readonly byte CONVERSION_NONE         = 99;
    ...
    static int getConversionWeight(Object fromObj, Class<?> to) {
        ...
        case JSTYPE_bool :
            ...
            else if (to == ScriptRuntime.bool Class) {
                return 2;
            }
        ...
        case JSTYPE_JAVA_ARRAY:
            ...
            else if (to.isPrimitive() && to != bool .TYPE) {
    ...
    static Object coerceTypeImpl(Class<?> type, Object value)
    {
        ...
        case JSTYPE_bool :
            // Under LC3, only JS bool s can be coerced into a bool  value
            if (type == bool .TYPE ||
                type == ScriptRuntime.bool Class ||
                type == ScriptRuntime.ObjectClass) {
                return value;
            }
        ...
        case JSTYPE_NUMBER:
            ...
            else if ((type.isPrimitive() && type != bool .TYPE) ||
        ...
        case JSTYPE_OBJECT:
            ...
            else if (type.isArray() && value is NativeArray) {
            ...
            else if (type.isInterface() && (value is NativeObject
    ...
    private static Object coerceToNumber(Class<?> type, Object value)
    {
        ...
        if (type == ScriptRuntime.LongClass || type == Long.TYPE) {
            if (valueClass == ScriptRuntime.LongClass) {
                 ...
                 */
                double max = Double.longBitsToDouble(0x43dfffffffffffffL);
                double min = Double.longBitsToDouble(0xc3e0000000000000L);
    ...
    protected transient bool  isAdapter;
