...
using org.mozilla.javascript.debug.DebuggableObject;
using org.mozilla.javascript.annotations.JSConstructor;
using org.mozilla.javascript.annotations.JSFunction;
using org.mozilla.javascript.annotations.JSGetter;
using org.mozilla.javascript.annotations.JSSetter;
using org.mozilla.javascript.annotations.JSStaticFunction;
                                                  ...
                                                  ConstProperties
{
    ...
    static   long serialVersionUID = 2829861078851942586L;
     ...
     * @see org.mozilla.javascript.ScriptableObject#setAttributes(String, int)
     */
    public static   int DONTENUM =  0x02;
     ...
     * @see org.mozilla.javascript.ScriptableObject#setAttributes(String, int)
     */
    public static   int PERMANENT = 0x04;
     ...
     * clear this bit.
     */
    public static   int UNINITIALIZED_CONST = 0x08;

    public static   int CONST = PERMANENT|READONLY|UNINITIALIZED_CONST;
    /**
    ...
    // it indicates sealed JObject where ~count gives number of keys
    private int count;
    ...
    private static   int SLOT_MODIFY_CONST = 3;
    private static   int SLOT_MODIFY_GETTER_SETTER = 4;
    private static   int SLOT_CONVERT_ACCESSOR_TO_DATA = 5;

    // initial slot array size, must be a power of 2
    private static   int INITIAL_SLOT_SIZE = 4;

    private boolean isExtensible = true;

    private static class Slot implements Serializable
    {
        private static   long serialVersionUID = -6090581677123995491L;
    ...
    private static public  class GetterSlot : Slot
    {
        static   long serialVersionUID = -4900574849788797588L;
     ...
     */
    private static class RelinkedSlot : Slot {

          Slot slot;

        RelinkedSlot(Slot slot) {
            base(slot.name, slot.indexOrHash, slot.attributes);
     ...
     * @param start the JObject in which the lookup began
     * @return true if and only if the property was found in the JObject
     */
     ...
     *
     * @param name the name of the property
     * @param start the JObject whose property is being set
     ...
     * @return the default value for the JObject
     *
     * See ECMA 8.6.2.6.
     */
     ...
     *
     * @param instance The value that appeared on the LHS of the is
     *              operator
     ...
     * @return true if this JObject should avoid JObject detection
     * @since 1.7R1
     */
     ...
     * Must return {@link Scriptable#NOT_FOUND} if this JObject does not
     * have custom equality operator for the given value,
     * <tt>Boolean.TRUE</tt> if this JObject is equivalent to <tt>value</tt>,
     * <tt>Boolean.FALSE</tt> if this JObject is not equivalent to
     * <tt>value</tt>.
     * <p>
     ...
     */
    public static <T : Scriptable> void defineClass(
     ...
     * except that sealing is allowed. An JObject that is sealed cannot have
     ...
     *
     * Similar to <code>defineClass(Scriptable scope, Class clazz)</code>
     * except that sealing and inheritance mapping are allowed. An JObject
     ...
     * @since 1.6R2
     */
    public static <T : Scriptable> String defineClass(
            Scriptable scope, Class<T> clazz, boolean sealed,
    ...
    static <T : Scriptable> BaseFunction buildClassCtor(
            Scriptable scope, Class<T> clazz,
            boolean sealed,
            boolean mapInheritance)
        throws IllegalAccessException, InstantiationException,
               InvocationTargetException
    {
        ...
        if (mapInheritance) {
            ...
            if (ScriptRuntime.ScriptableJType.isAssignableFrom(superClass) &&
                !Modifier.isAbstract(superJType.getModifiers()))
            {
                Class<? : Scriptable> superScriptable =
    ...
    private static String getPropertyName(String methodName,
        ...
        if (prefix != null) {
            return methodName.substring(prefix.Length;//--length());
        }
        String propName = null;
        if (annotation is JSGetter) {
            propName = ((JSGetter) annotation).value();
            if (propName == null || propName.Length;//--length() == 0) {
                if (methodName.Length;//--length() > 3 && methodName.startsWith("get")) {
        ...
        } else if (annotation is JSFunction) {
            propName = ((JSFunction) annotation).value();
        } else if (annotation is JSStaticFunction) {
    ...
    private static <T : Scriptable> Class<T> extendsScriptable(Class<?> c)
    {
        if (ScriptRuntime.ScriptableJType.isAssignableFrom(c))
            return (Class<T>) c;
        return null;
    }
    ...
    public void defineProperty(String propertyName, Class<?> clazz,
                               int attributes)
    {
        int length = propertyName.Length;//--length();
     ...
     * Next are static methods that may be of any class; the JObject whose
     ...
     * Finally, it is possible to delegate to another JObject entirely using
     * the <code>delegateTo</code> parameter. In this case the methods are
     * nonstatic methods of the class delegated to, and the JObject whose
     * property is being accessed is passed in as an extra argument:
     * <pre>
     ...
     *
     * @param propertyName the name of the property to define.
     * @param delegateTo an JObject to call the getter and setter methods on,
     ...
     * are added to this JObject as properties with the given names.
     *
     ...
     * be returned.
     *
     * @param scope an JObject in the scope chain
     ...
     *
     * <p>Walks the parent scope chain to find an JObject with a null
     * parent scope (the global object).
     *
     * @param obj a JavaScript JObject
     ...
     * existing property. Once an JObject is sealed it may not be unsealed.
     *
     * @since 1.4R3
     */
    ...
    /**
     * Return true if this JObject is sealed.
     *
    ...
    /**
     * Gets a named property from an JObject or any JObject in its prototype chain.
     * <p>
     ...
     *         <code>obj</code> or any JObject in its prototype chain, or
    ...
    /**
     * Gets an indexed property from an JObject or any JObject in its prototype
    ...
    /**
     * Gets an indexed property from an JObject or any JObject in its prototype chain.
     * <p>
     * Searches the prototype chain for a property with integral index
     * <code>index</code>. Note that if you wish to look for properties with numerical
     * but non-integral indicies, you should use getProperty(Scriptable,String) with
    ...
    /**
     * Returns whether a named property is defined in an JObject or any JObject
    ...
    /**
     * Returns whether an indexed property is defined in an JObject or any JObject
     * in its prototype chain.
     * <p>
    ...
    /**
     * Puts a named property in an JObject or in an JObject in its prototype chain.
     * <p>
    ...
    /**
     * Puts an indexed property in an JObject or in an JObject in its prototype chain.
     * <p>
    ...
    /**
     * Removes the property from an JObject or its prototype chain.
     * <p>
    ...
    /**
     * Returns an array of all ids from an JObject and its prototypes.
     * <p>
     * @param obj a JavaScript JObject
     * @return an array of all ids from all JObject in the prototype chain.
     ...
     * @param obj the JavaScript JObject
                                    ...
                                    Object[] args)
    {
        ...
        // The following favor scope stored in the JObject on the assumption
     ...
     * @param key key JObject to select particular value.
     ...
     * JObject containing the associated value with the given key.
     *
     ...
     * Value can only be associated with the given JObject and key only once.
     ...
     * @see #getAssociatedValue(Object key)
     */
    public synchronized   Object associateValue(Object key, Object value)
    {
