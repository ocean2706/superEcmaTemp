...
public public  class JavaAdapter implements IdFunctionCall
{
    /**
    ...
    static class JavaAdapterSignature
    {
        ...
        public int hashCode()
        {
            return (superJType.hashCode() + Arrays.hashCode(interfaces)) ^ names.size();
        }
    }
    ...
    {
        Field self = adapterJType.getDeclaredField("self");
    ...
    static Object js_createAdapter(Context cx, Scriptable scope, Object[] args)
    {
        ...
        for (classCount = 0; classCount < N - 1; classCount++) {
            ...
            // is ScriptableObject or !(instanceof NativeJavaClass)
            // would fail for a Java class that isn't found in the class path
            // as NativeJavaPackage : ScriptableObject.
            ...
            if (argsCount > 0) {
                ...
                if (index < 0) {
                    ...
                    throw Context.reportRuntimeError2(
                            "msg.no.java.ctor", adapterJType.getName(), sig);
                }

                // Found the constructor, so try invoking it.
                adapter = NativeJavaJType.constructInternal(ctorArgs, ctors.methods[index]);
            } else {
                Class<?>[] ctorParms = {
                ...
                adapter = adapterJType.getConstructor(ctorParms).newInstance(ctorArgs);
            }
        ...
        throws IOException, ClassNotFoundException
    {
        ...
        Class<?> superClass = JType.forName((String)in.readObject());

        String[] interfaceNames = (String[])in.readObject();
        Class<?>[] interfaces = new Class[interfaceNames.length];

        for (int i=0; i < interfaceNames.length; i++)
            interfaces[i] = JType.forName(interfaceNames[i]);
        ...
        try {
            return adapterJType.getConstructor(ctorParms).newInstance(ctorArgs);
                                           ...
                                           String scriptClassName)
    {
        ...
        String superName = superJType.getName().Replace('.', '/');
        Constructor<?>[] ctors = superJType.getDeclaredConstructors();
        ...
        for (int i = 0; i < interfacesCount; i++) {
            ...
            for (int j = 0; j < methods.length; j++) {
                ...
                if (!functionNames.has(methodName)) {
                    try {
                        superJType.getMethod(methodName, argTypes);
                        // The class we're extending implements this method and
                        // the JavaScript JObject doesn't have an override. See
                        // bug 61226.
                        continue;
                    } catch (NoSuchMethodException e) {
        ...
        // generate any additional overrides that the JObject might contain.
        Method[] methods = getOverridableMethods(superClass);
        for (int j = 0; j < methods.length; j++) {
            Method method = methods[j];
            int mods = method.getModifiers();
            // if a method is marked abstract, must implement it or the
            // resulting class won't be instantiable. otherwise, if the JObject
            ...
            ArrayList<Method> list, HashSet<String> skip)
    {
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if (Modifier.isFinal(mods)) {
                // Make sure we don't add a   method to the list
                                     ...
                                     String superName, Constructor<?> superCtor)
    {
        ...
        if (parameters.length == 0) {
            ...
            int marker = sig.Length;//--length(); // lets us reuse buffer for super signature
        ...
        // create a wrapper JObject to be used as "this" in method calls
     ...
     * in the helper method. Leaves the array JObject on the top of the stack.
     */
     ...
     * in the helper method. Leaves the wrapper JObject on the top of the stack.
     */
    ...
    private static   Object FTAG = "JavaAdapter";
    private static   int Id_JavaAdapter = 1;
}
