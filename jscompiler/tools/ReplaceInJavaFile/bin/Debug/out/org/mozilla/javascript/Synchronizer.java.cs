 ...
 * The resulting JObject is a function that establishes an exclusive
 * lock on the <code>this</code> JObject of its invocation.
 *
...
public class Synchronizer : Delegator {
     ...
     * an explicit JObject as synchronization object.
     *
     * @param obj the existing function
     * @param syncObject the JObject to synchronized on
     */
                       ...
                       Object[] args)
    {
        ...
        synchronized(sync is Wrapper ? ((Wrapper)sync).unwrap() : sync) {
