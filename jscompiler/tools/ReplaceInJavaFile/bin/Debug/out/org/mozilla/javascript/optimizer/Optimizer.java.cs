...
class Optimizer
{
    ...
    static   int NoType = 0;
    static   int NumberType = 1;
    static   int AnyType = 3;
...
/*
        Each directCall parameter is passed as a pair of values - an JObject
        and a double. The value passed depends on the type of value available at
        the call site. If a double is available, the JObject in java/lang/Void.TYPE
        is passed as the JObject value, and if an JObject value is available, then
        0.0 is passed as the double value.

        The receiving routine always tests the JObject value before proceeding.
