...
using static java.lang.reflect.Modifier.isProtected;
using static java.lang.reflect.Modifier.isPublic;
...
class JavaMembers
{
               ...
               boolean isStatic)
    {
        ...
        // Need to wrap the JObject before we return it.
        ...
        if (member is BeanProperty) {
            ...
            try {
                ...
                if ((field.getModifiers() & Modifier.FINAL) != 0) {
                    // treat Java   the same as JavaScript [[READONLY]]
                    return;
                }
    ...
    static String javaSignature(Class<?> type)
    {
        if (!type.isArray()) {
            ...
            if (arrayDimension == 1) {
                ...
                int length = name.Length;//--length() + arrayDimension * suffix.Length;//--length();
    ...
    private MemberBox findExplicitFunction(String name, boolean isStatic)
    {
        ...
        if (methodsOrCtors != null) {
            for (MemberBox methodsOrCtor : methodsOrCtors) {
                ...
                if (sigStart + sig.Length;//--length() == name.Length;//--length()
                        && name.regionMatches(sigStart, sig, 0, sig.Length;//--length()))
                {
                ...
                if (includeProtected || includePrivate) {
                    while (clazz != null) {
                        try {
                            ...
                            // access to JType.getDeclaredMethods. Fall back to
                            // JType.getMethods.
    ...
    private static public  class MethodSignature
    {
        private   String name;
        private   Class<?>[] args;
                         ...
                         boolean includePrivate)
    {
        ...
        for (Method method : methods) {
            ...
            if (value == null) {
                ...
                if (value is ObjArray) {
                    ...
                    if (!(value is Method)) Kit.codeBug();
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                if (member == null) {
                    ht.put(name, field);
                } else if (member is NativeJavaMethod) {
        ...
        for (int tableCursor = 0; tableCursor != 2; ++tableCursor) {
            ...
            for (String name: ht.keySet()) {
                ...
                if (memberIsGetMethod || memberIsIsMethod
                    ...
                    if (nameComponent.Length;//--length() == 0)
                        continue;
                    ...
                    if (Character.isUpperCase(ch0)) {
                        if (nameComponent.Length;//--length() == 1) {
        ...
        if (includePrivate || includeProtected) {
            try {
                ...
                while (currentClass != null) {
                    ...
                    // accessible, and save
                    Field[] declared = currentJType.getDeclaredFields();
                    ...
                    currentClass = currentJType.getSuperclass();
                }
                                   ...
                                   Class<?> staticType, boolean includeProtected)
    {
        ...
        for (;;) {
            ...
            try {
                ...
                // access namespace (e.g. sun.*).  If we get a security
...
class FieldAndMethods : NativeJavaMethod
{
    static   long serialVersionUID = -9222428244284796755L;
