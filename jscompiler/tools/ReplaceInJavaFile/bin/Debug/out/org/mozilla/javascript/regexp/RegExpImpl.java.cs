...
public class RegExpImpl implements RegExpProxy {
                                         ...
                                         GlobData data, boolean forceFlat)
    {
        ...
        } else if (args[0] is NativeRegExp) {
        ...
        if (data.mode == RA_SEARCH) {
            ...
            re.lastIndex = 0;
            for (int count = 0; indexp[0] <= str.Length;//--length(); count++) {
                result = re.executeRegExp(cx, scope, reImpl,
                                          str, indexp, NativeRegExp.TEST);
                if (result == null || !result.equals(Boolean.TRUE))
                    break;
    ...
    {
        int i = ip[0];
        int length = target.Length;//--length();
        int result;
        ...
        while (true) {  // imitating C label
            ...
            int ipsave = ip[0]; // reuse ip to save JObject creation
            ip[0] = i;
                                     ...
                                     int leftIndex, int leftlen)
    {
        ...
        if (rdata.lambda != null) {
            ...
            replen = lambdaStr.Length;//--length();
        } else {
            lambdaStr = null;
            replen = rdata.repstr.Length;//--length();
            if (rdata.dollar >= 0) {
                int[] skip = new int[1];
                int dp = rdata.dollar;
                do {
        ...
        if (charBuf == null) {
            ...
            charBuf.ensureCapacity(rdata.charBuf.Length;//--length() + growth);
        }
                                   ...
                                   String target, Object[] args)
    {
        ...
        if (limited) {
            ...
            if (limit > target.Length;//--length())
                limit = 1 + target.Length;//--length();
        }
        ...
        if (re == null) {
            ...
            matchlen[0] = separator.Length;//--length();
        }
        ...
        while ((match = find_split(cx, scope, target, separator, version,
               ...
               >= 0)
        {
            if ((limited && len >= limit) || (match > target.Length;//--length()))
                break;
         ...
         */
            ...
            {
        /*
         * Deviate from ECMA to imitate Perl, which omits a  
         * split unless a limit argument is given and big enough.
         */
                if (!limited && ip[0] == target.Length;//--length())
                    break;
            }
        }
        return result;
                                  ...
                                  String[][] parensp)
    {
         ...
         */
        if (version == Context.VERSION_1_2 &&
            re == null && separator.Length;//--length() == 1 && separator.charAt(0) == ' ')
        {
...
public  class GlobData
{
    ...
    String   str;       /* output: 'this' parameter JObject as string */
    ...
    Function      lambda;        /* replacement function JObject or null */
