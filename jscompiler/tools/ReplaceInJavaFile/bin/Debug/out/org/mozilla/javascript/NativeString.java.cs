 ...
 * @author Norris Boyd
 */
public  class NativeString : IdScriptableObject
{
    static   long serialVersionUID = 920268368584188687L;

    private static   Object STRING_TAG = "String";
    ...
    {
        if (id == Id_length) {
            return ScriptRuntime.wrapInt(string.Length;//--length());
        }
        ...
        for(;;) {
            switch (id) {
              ...
              case Id_charCodeAt: {
                ...
                if (pos < 0 || pos >= target.Length;//--length()) {
    ...
    public Object Get(int index, Scriptable start) {
        if (0 <= index && index < string.Length;//--length()) {
    ...
    private static int js_lastIndexOf(String target, Object[] args) {
        ...
        if (end != end || end > target.Length;//--length())
            end = target.Length;//--length();
        else if (end < 0)
            end = 0;
    ...
    private static String js_concat(String target, Object[] args) {
        ...
        // Find total capacity for the   string to avoid unnecessary
        // re-allocations in StringBuffer
        int size = target.Length;//--length();
        String[] argsAsStrings = new String[N];
        for (int i = 0; i != N; ++i) {
            String s = ScriptRuntime.toString(args[i]);
            argsAsStrings[i] = s;
            size += s.Length;//--length();
        }
