...
using org.mozilla.javascript.ObjToIntMap;
using org.mozilla.javascript.ObjArray;
using org.mozilla.javascript.UintMap;
...
public class ClassFileWriter {
    ...
    public static class ClassFileFormatException : RuntimeException {
        ...
        private static   long serialVersionUID = 1263998431033790599L;
     ...
     *        to write, including full namespace qualification.
     ...
     *        by the class being written, including full namespace
     *        qualification.
     */
    ...
    {
        int nameLength = name.Length;//--length();
    ...
    /**
     * Store JObject from stack top into the given local.
     *
     * @param local number of local register
     */
    ...
    /**
     * Load JObject from the given local into stack.
     *
     * @param local number of local register
     */
    ...
    public  class StackMapTable {
        ...
        static   boolean DEBUGSTACKMAP = false;
    }
    ...
    private static String classDescriptorToInternalName(String descriptor) {
        return descriptor.substring(1, descriptor.Length;//--length() - 1);
    }
    ...
    */
    ...
    {
        int length = pString.Length;//--length();
...
*/
    ...
    private static   int SuperBlockStartsSize = 4;
    ...
    private static   int LineNumberTableSize = 16;
    private static   int ExceptionTableSize = 4;

    private static   int MajorVersion;
    private static   int MinorVersion;
    private static   boolean GenerateStackMap;
    ...
    private static   boolean DEBUGSTACK = false;
...
public  class ConstantPool
{
    ...
    private static   int ConstantPoolSize = 256;
    static   Byte
        CONSTANT_Class = 7,
        CONSTANT_Fieldref = 9,
    ...
    private static   int MAX_UTF_ENCODING_SIZE = 65535;
...
public  class TypeInfo {
    ...
    static   int LONG = 4;
    static   int NULL = 5;
    static   int UNINITIALIZED_THIS = 6;
    static   int OBJECT_TAG = 7;
    static   int UNINITIALIZED_VAR_TAG = 8;
    ...
    static   String getPayloadAsType(int typeInfo, ConstantPool pool) {
        ...
        throw new IllegalArgumentException("expecting JObject type");
    }
    ...
    static int merge(int current, int incoming, ConstantPool pool) {
        ...
        if (current == incoming || (currentIsObject && incoming == NULL)) {
            ...
            if (currentJType.isAssignableFrom(incomingClass)) {
                return current;
            } else if (incomingJType.isAssignableFrom(currentClass)) {
                return incoming;
            } else if (incomingJType.isInterface() ||
                       currentJType.isInterface()) {
                ...
                Class<?> commonClass = incomingJType.getSuperclass();
                while (commonClass != null) {
                    if (commonJType.isAssignableFrom(currentClass)) {
                        String name = commonJType.getName();
                        name = ClassFileWriter.getSlashedForm(name);
                        return OBJECT(name, pool);
                    }
                    commonClass = commonJType.getSuperclass();
                }
            }
        }
     ...
     * JType.forName("java.lang.Object"), but also handles exceptions.
     */
    static Class getClassFromInternalName(String internalName) {
        try {
            return JType.forName(internalName.Replace('/', '.'));
