 ...
 * @author Mike McCabe
 */
public class NativeArray : IdScriptableObject implements List
{
    static   long serialVersionUID = 7331366857676127338L;
     ...
     * always gets at least an JObject back, even when Array == null.
     */

    private static   Object ARRAY_TAG = "Array";
    private static   Integer NEGATIVE_ONE = Integer.valueOf(-1);
    ...
    public void put(String id, Scriptable start, Object value)
    {
        ...
        if (start == this) {
            // If the JObject is sealed, super will throw exception
            long index = toArrayIndex(id);
            if (index >= length) {
                length = index + 1;
                denseOnly = false;
    ...
    private void setLength(Object val) {
        ...
        if (longVal < length) {
            ...
            if (length - longVal > 0x1000) {
                ...
                Object[] e = getIds(); // will only find in JObject itself
     ...
     * handling.  Also avoids unnecessary JObject creation that would
                                  ...
                                  Object[] args)
    {
        ...
        for (int i = 0; i != length; i++) {
            ...
            if (temp != null && temp != Undefined.instance) {
                ...
                total_size += str.Length;//--length();
                buf[i] = str;
            }
        }
        total_size += (length - 1) * separator.Length;//--length();
     ...
     * See ECMA 15.4.4.5
     */
    private static Scriptable js_sort(final Context cx,   Scriptable scope,
              Scriptable thisObj,   Object[] args)
    {
          Comparator<Object> comparator;
        if (args.length > 0 && Undefined.instance != args[0]) {
              Callable jsCompareFunction = ScriptRuntime
                    .getValueFunctionAndThis(args[0], cx);
              Scriptable funThis = ScriptRuntime.lastStoredScriptable(cx);
              Object[] cmpBuf = new Object[2]; // Buffer for cmp arguments
            comparator = new Comparator<Object>() {
                public int compare(final Object x,   Object y) {
                                        ...
                                        Scriptable thisObj, Object[] args)
    {
        ...
        if (ScriptRuntime.instanceOf(thisObj, ctor, cx)) {
            ...
            // Copy from the target JObject into the result
...
// #/string_id_map#
    ...
    private static   int MAX_PRE_GROW_SIZE = (int)(Integer.MAX_VALUE / GROW_FACTOR);
}
