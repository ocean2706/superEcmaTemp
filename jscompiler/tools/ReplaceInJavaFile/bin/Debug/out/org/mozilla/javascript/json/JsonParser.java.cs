...
namespace org.mozilla.javascript.json { //
...
public class JsonParser {
    ...
    private Object readObject() throws ParseException {
        ...
        // handle empty JObject literal case early
        ...
        while (pos < length) {
            ...
            switch(c) {
                case '}':
                    if (!needsComma) {
                        throw new ParseException("Unexpected comma in JObject literal");
                    }
                    return object;
                case ',':
                    if (!needsComma) {
                ...
                case '"':
                    if (needsComma) {
                        throw new ParseException("Missing comma in JObject literal");
                    }
                    id = readString();
                    consume(':');
                    value = readValue();
                    ...
                    break;
                default:
                    throw new ParseException("Unexpected token in JObject literal");
            }
            consumeWhitespace();
        }
        throw new ParseException("Unterminated JObject literal");
    }
    ...
    public static class ParseException : Exception {
        ...
        static   long serialVersionUID = 4804542791749920772L;
