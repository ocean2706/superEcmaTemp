...
public class Decompiler
{
    ...
    // the last RC of JObject literals in case of function expressions
    private static   int FUNCTION_END = Token.LAST_TOKEN + 1;
    ...
    public static String decompile(String source, int flags,
                                   UintMap properties)
    {
        int length = source.Length;//--length();
        if (length == 0) { return ""; }
        ...
        if (printSource) {
            ...
            for (int i = 0; i < length; ++i) {
                ...
                String pad = tokenname.Length;//--length() > 7
                    ? "\t"
                    : "\t\t";
                System.err.println
                    (tokenname
