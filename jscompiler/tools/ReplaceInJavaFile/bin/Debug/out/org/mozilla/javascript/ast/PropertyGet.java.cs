...
public class PropertyGet : InfixExpression {
    ...
    public PropertyGet(AstNode target, Name property, int dotPosition) {
        base(Token.GETPROP, target, property, dotPosition);
    }

    /**
     * Returns the JObject on which the property is being fetched.
     ...
     * @param target expression evaluating to the JObject upon which
