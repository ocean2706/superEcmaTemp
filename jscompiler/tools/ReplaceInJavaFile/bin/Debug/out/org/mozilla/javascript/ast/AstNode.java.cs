 ...
 * Java or C++. <p>
 */
public abstract class AstNode : Node implements Comparable<AstNode> {
     ...
     *
     * @param visitor the JObject to call with this node and its children
     */
     ...
     */
    protected <T : AstNode> void printList(List<T> items,
                                                 StringBuilder sb) {
        int max = items.size();
        int count = 0;
...
//     }
    ...
    protected static class DebugPrintVisitor implements NodeVisitor {
        ...
        private static   int DEBUG_INDENT = 2;
