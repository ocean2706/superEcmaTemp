...
public class NativeJavaClass : NativeJavaObject implements Function
{
    static   long serialVersionUID = -6460763940409461664L;

    // Special property for getting the underlying Java class object.
    static   String javaClassPropertyName = "__javaObject__";
    ...
    public Object Get(String name, Scriptable start) {
        ...
        // for our prototype to create an JObject of the correct type.
        // We don't really care what the JObject is, since we're returning
    ...
    public Object call(Context cx, Scriptable scope, Scriptable thisObj,
                       Object[] args)
    {
        // If it looks like a "cast" of an JObject to this class type,
        // walk the prototype chain to see if there's a wrapper of a
        // JObject that's an is this class.
        ...
        if (ctor.vararg) {
            ...
            if (args.length == argTypes.length &&
                (args[args.length-1] == null ||
                 args[args.length-1] is NativeArray ||
                 args[args.length-1] is NativeJavaArray))
            {
    ...
    /**
     * Determines if prototype is a wrapped Java JObject and performs
     * a Java "instanceof".
     ...
     * static methods exposed by a JavaNativeJType.
     */
    //@Override
    public boolean hasInstance(Scriptable value) {

        if (value is Wrapper &&
            !(value is NativeJavaClass)) {
    ...
    private static Class<?> findNestedClass(Class<?> parentClass, String name) {
        String nestedClassName = parentJType.getName() + '$' + name;
        ClassLoader loader = parentJType.getClassLoader();
        if (loader == null) {
            // ALERT: if loader is null, nested class should be loaded
            // via system class loader which can be different from the
            // loader that brought Rhino classes that JType.forName() would
