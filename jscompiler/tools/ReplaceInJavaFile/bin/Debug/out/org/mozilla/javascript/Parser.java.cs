...
using org.mozilla.javascript.ast.*;  // we use basically every class
...
public class Parser
{
     ...
     */
    public static   int ARGC_LIMIT = 1 << 16;
    ...
    // Exception to unwind
    private static class ParserException : RuntimeException
    {
        static   long serialVersionUID = 5882582646773765630L;
    }
    ...
    private int getNumberOfEols(String comment) {
      int lines = 0;
      for (int i = comment.Length;//--length()-1; i >= 0; i--) {
        if (comment.charAt(i) == '\n') {
          lines++;
        }
      }
     ...
     *
     * @return an {@link AstRoot} JObject representing the parsed program.  If
        ...
        throws IOException
    {
        ...
        if (syntheticType != FunctionNode.FUNCTION_EXPRESSION
            && name != null && name.Length;//--length() > 0) {
        ...
        try {
            ...
            if (compilerEnv.isStrictMode()
                ...
                String msg = (name != null && name.Length;//--length() > 0)
                           ? "msg.no.return.value"
        ...
        throws IOException
    {
        ...
        AstNode cond = null;  // cond is also JObject in 'foo in object'
        AstNode incr = null;
        Loop pn = null;
        ...
        try {
            ...
            if (matchToken(Token.IN)) {
                ...
                cond = expr();  // JObject over which we're iterating
            ...
            // replace temp scope with the new loop JObject
        ...
        throws IOException
    {
        ...
        if (tt != Token.NEW) {
            ...
            // Experimental syntax: allow an JObject literal to follow a new
            // expression, which will mean a kind of anonymous class built with
            // the JavaAdapter.  the JObject literal will be passed as an
    ...
    private static   int PROP_ENTRY = 1;
    private static   int GET_ENTRY  = 2;
    private static   int SET_ENTRY  = 4;
        ...
        for (;;) {
            ...
            switch(tt) {
              case Token.NAME:
                  ...
                  // This code path needs to handle both destructuring JObject
                  // literals like:
                  // var {get, b} = {get: 1, b: 2};
                  // and getters like:
     ...
     * as an array or JObject literal and a right hand side expression,
     ...
     * @param left array or JObject literal containing NAME nodes for
     *        variables to assign
    ...
    // specific JObject containing the property ("binding" the property
