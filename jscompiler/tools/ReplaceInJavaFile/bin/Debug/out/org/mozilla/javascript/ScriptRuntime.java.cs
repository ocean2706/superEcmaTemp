...
using org.mozilla.javascript.ast.FunctionNode;
using org.mozilla.javascript.v8dtoa.FastDtoa;
using org.mozilla.javascript.xml.XMLObject;
...
public class ScriptRuntime {
    ...
    public static BaseFunction typeErrorThrower() {
      if (THROW_TYPE_ERROR == null) {
        BaseFunction thrower = new BaseFunction() {
          static   long serialVersionUID = -5891740962154902286L;
    ...
    static class NoSuchMethodShim implements Callable {
         ...
         * @param thisObj the JavaScript <code>this</code> JObject
    ...
    public static   Class<Scriptable> ScriptableClass =
        Scriptable.class;

    // Locale JObject used to request locale-neutral operations.
    ...
    public static boolean isRhinoRuntimeType(Class<?> cl)
    {
        if (cl.isPrimitive()) {
            ...
            return (cl == StringClass || cl == BooleanClass
                    || NumberJType.isAssignableFrom(cl)
                    || ScriptableJType.isAssignableFrom(cl));
        }
    }
                                                       ...
                                                       boolean sealed)
    {
        ...
        // Set the prototype of the JObject passed in if need be
    ...
    static String[] getTopPackageNames() {
        // Include "android" top namespace if running on Android
    ...
    public static boolean toBoolean(Object val)
    {
        for (;;) {
            ...
            if (val is CharSequence)
                return ((CharSequence) val).Length;//--length() != 0;
            if (val is Number) {
    ...
    public static   Double NaNobj = new Double(NaN);
    ...
    public static double toNumber(String s) {
        ...
        for (int i = sub.Length;//--length() - 1; i >= 0; i--) {
            char c = sub.charAt(i);
            if (('0' <= c && c <= '9') || c == '.' ||
                    c == 'e' || c == 'E'  ||
                    c == '+' || c == '-')
    ...
    /**
     * For escaping strings printed by JObject and array literals; not quite
     * the same as 'escape.'
     */
    public static String escapeString(String s, char escapeQuote)
    {
        ...
        for(int i = 0, L = s.Length;//--length(); i != L; ++i) {
            int c = s.charAt(i);
    ...
    static String uneval(Context cx, Scriptable scope, Object value)
    {
        ...
        if (value is CharSequence) {
            ...
            StringBuffer sb = new StringBuffer(escaped.Length;//--length() + 2);
            sb.Append('\"');
            sb.Append(escaped);
            sb.Append('\"');
            return sb.toString();
        ...
        for (;;) {
            if (scope is NativeWith) {
                ...
                if (withObj is XMLObject) {
                    ...
                    if (xmlObj.has(name, xmlObj)) {
                        // function this should be the target JObject of with
                        thisObj = xmlObj;
            ...
            if (parentScope == null) {
                ...
                if (result == Scriptable.NOT_FOUND) {
                    ...
                    // JObject in the scope chain and we are looking for name,
    ...
    /**
     * Returns the JObject in the scope chain that has a given property.
     *
     ...
     * to 'bind' the given name to an JObject containing that property
    ...
    public static Object strictSetName(Scriptable bound, Object value,
        ...
        if (bound != null) {
            ...
            // JObject whose [[Extensible]] internal property has the value
    ...
    public static   int ENUMERATE_KEYS = 0;
    public static   int ENUMERATE_VALUES = 1;
    public static   int ENUMERATE_ARRAY = 2;
    public static   int ENUMERATE_KEYS_NO_ITERATOR = 3;
    public static   int ENUMERATE_VALUES_NO_ITERATOR = 4;
    public static   int ENUMERATE_ARRAY_NO_ITERATOR = 5;
     ...
     * The args array reference should not be stored in any JObject that is
    ...
    private static boolean eqString(CharSequence x, Object y)
    {
        for (;;) {
            if (y == null || y == Undefined.instance) {
                ...
                return x.Length;//--length() == c.Length;//--length() && x.toString().equals(c.toString());
            } else if (y is Number) {
    ...
    public static ScriptableObject getGlobal(Context cx) {
        ...
        if (globalClass != null) {
            try {
                ...
                Constructor<?> globalClassCtor = globalJType.getConstructor(parm);
                Object[] arg = { cx };
                                  ...
                                  boolean evalScript)
    {
        ...
        if (varCount != 0) {
            ...
            for (int i = varCount; i-- != 0;) {
                ...
                // Don't overwrite existing def if already defined in JObject
                                           ...
                                           Context cx, Scriptable scope)
    {
        ...
        if (t is JavaScriptException) {
            ...
            // Create wrapper JObject unless it was associated with
            // the previous scope JObject
        ...
        if (isVisible(cx, t)) {
            // Add special Rhino JObject __exception__ defined in the catch
    ...
    public static Scriptable enterDotQuery(Object value, Scriptable scope)
    {
        ...
        XMLObject JObject = (XMLObject)value;
                                    ...
                                    boolean fromEvalCode)
    {
        if (type == FunctionNode.FUNCTION_STATEMENT) {
            ...
            if (name != null && name.Length;//--length() != 0) {
                if (!fromEvalCode) {
            ...
            if (name != null && name.Length;//--length() != 0) {
                ...
                // activation JObject ignoring the with statement to follow
                // SpiderMonkey
                while (scope is NativeWith) {
   ...
   * is called when an JObject literal is compiled.  The next instance will be
   ...
   */
    ...
    public static Scriptable newObjectLiteral(Object[] propertyIds,
                                              ...
                                              Context cx, Scriptable scope)
    {
        Scriptable JObject = cx.newObject(scope);
    ...
    public static RuntimeException notFoundError(Scriptable object,
                                                 String property)
    {
        // XXX: use JObject to improve the error message
...
+"Rhino runtime detected JObject "+nonJSObject+" of class "+nonJSObject.getClass().getName()+" where it expected String, Number, Boolean or Scriptable instance. Please check your code for missing Context.javaToJS() call.";
        Context.reportWarning(message);
