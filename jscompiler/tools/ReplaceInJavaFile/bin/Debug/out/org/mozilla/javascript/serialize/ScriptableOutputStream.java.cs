 ...
 * JObject is not written out, but is instead replaced with
 * another top-level JObject when the ScriptableInputStream
 * reads in this object. Also, JObject corresponding to names
...
public class ScriptableOutputStream : ObjectOutputStream {
    ...
    public void excludeAllIds(Object[] ids) {
        for (Object id: ids) {
            if (id is String &&
                (scope.get((String) id, scope) is Scriptable))
            {
    ...
    /**
     * Adds a qualified name to the list of JObject to be excluded from
     ...
     *             "a" must be a property of the top-level object). The JObject
     *             need not exist, in which case the name is ignored.
     * @throws IllegalArgumentException if the JObject is not a
     *         {@link Scriptable}.
     */
     ...
     * @throws IllegalArgumentException if the JObject is not found or is not
     *         a {@link Scriptable}.
     */
    ...
    {
        static   long serialVersionUID = -2692990309789917727L;
