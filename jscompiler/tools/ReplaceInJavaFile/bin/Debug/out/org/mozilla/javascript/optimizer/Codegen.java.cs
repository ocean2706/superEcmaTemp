...
namespace org.mozilla.javascript.optimizer { //

using org.mozilla.javascript.*;
using org.mozilla.javascript.ast.FunctionNode;
using org.mozilla.javascript.ast.Jump;
using org.mozilla.javascript.ast.Name;
...
using static org.mozilla.classfile.ClassFileWriter.ACC_FINAL;
using static org.mozilla.classfile.ClassFileWriter.ACC_PRIVATE;
using static org.mozilla.classfile.ClassFileWriter.ACC_PUBLIC;
using static org.mozilla.classfile.ClassFileWriter.ACC_STATIC;
using static org.mozilla.classfile.ClassFileWriter.ACC_VOLATILE;
...
public class Codegen implements Evaluator
{
                          ...
                          boolean returnFunction)
    {
        ...
        String baseName = "c";
        if (tree.getSourceName().Length;//--length() > 0) {
                                 ...
                                 Object staticSecurityDomain)
    {
        ...
        throw new RuntimeException("Malformed optimizer namespace " + e) { //
    }
    ...
    private RuntimeException reportClassFileFormatException(
        ...
        String message)
    {
        String msg = scriptOrFn is FunctionNode
...
*/
    ...
    // One of these creates a generator JObject (NativeGenerator), which is
    ...
    private void generateMain(ClassFileWriter cfw)
    {
        ...
        // Call mainMethodJType.main(Script script, String[] args)
        cfw.addInvoke(ByteCode.INVOKESTATIC,
                      mainMethodClass,
                      "main",
    ...
    static RuntimeException badTree()
    {
     ...
     static   String DEFAULT_MAIN_METHOD_CLASS
        = "org.mozilla.javascript.optimizer.OptRuntime";

    private static   String SUPER_CLASS_NAME
        = "org.mozilla.javascript.NativeFunction";

    static   String ID_FIELD_NAME = "_id";

    static   String REGEXP_INIT_METHOD_NAME = "_reInit";
    static   String REGEXP_INIT_METHOD_SIGNATURE
        =  "(Lorg/mozilla/javascript/Context;)V";

    static   String FUNCTION_INIT_SIGNATURE
        =  "(Lorg/mozilla/javascript/Context;"
           +"Lorg/mozilla/javascript/Scriptable;"
           +")V";

   static   String FUNCTION_CONSTRUCTOR_SIGNATURE
        = "(Lorg/mozilla/javascript/Scriptable;"
...
class BodyCodegen
{
    void generateBodyCode()
    {
        ...
        // generate the body of the current function or script JObject
        initBodyGeneration();
    ...
    private void generateGenerator()
    {
        ...
        // create the NativeGenerator JObject that we return
        cfw.addALoad(variableObjectLocal);
    ...
    private void generatePrologue()
    {
        ...
        // skip creating activation JObject for the body of a generator. The
              ...
              case Token.CATCH_SCOPE:
                {
                    ...
                    child = child.getNext();
                    generateExpression(child, node); // load expression JObject
                    if (scopeIndex == 0) {
                        cfw.add(ByteCode.ACONST_NULL);
                    } else {
                        // Load previous catch scope JObject
                        cfw.addALoad(local);
                    }
                    cfw.addPush(name);
                    cfw.addALoad(contextLocal);
    ...
    private void generateExpression(Node node, Node parent)
    {
        ...
        switch (type) {
              ...
              case Token.GETELEM:
                generateExpression(child, node); // JObject
    ...
    private void visitFunction(OptFunctionNode ofn, int functionType)
    {
        ...
        if (functionType == FunctionNode.FUNCTION_EXPRESSION) {
            // Leave closure JObject on stack and do not pass it to
    ...
    private void visitObjectLiteral(Node node, Node child, boolean topLevel)
    {
        ...
        // check if JObject literal actually has any getters or setters
...
Else pass the JS JObject in the aReg and 0.0 in the dReg.
*/
    ...
    private void generateCallArgArray(Node node, Node argChild, boolean directCall)
    {
        ...
        // load array JObject to set arguments
    ...
    private void visitTryCatchFinally(Jump node, Node child)
    {
        ...
        if (catchTarget != null) {
            ...
            /*
                ...
                associated error JObject to the handler
            */
        ...
        if (finallyTarget != null) {
            ...
            // reset the variable JObject local
    ...
    private static   int JAVASCRIPT_EXCEPTION  = 0;
    private static   int EVALUATOR_EXCEPTION   = 1;
    private static   int ECMAERROR_EXCEPTION   = 2;
    private static   int THROWABLE_EXCEPTION   = 3;
    ...
    private static   int FINALLY_EXCEPTION = 4;
    private static   int EXCEPTION_MAX = 5;
                                    ...
                                    int handler)
    {
        ...
        // MS JVM gets cranky if the exception JObject is left on the stack
        cfw.addAStore(exceptionLocal);
    ...
    private void visitGetVar(Node node)
    {
        ...
        if (varIsDirectCallParameter(varIndex)) {
            ...
            // context, so test the JObject type and convert the
            //  value as necessary.
            if (node.getIntProp(Node.ISNUMBER_PROP, -1) != -1) {
                dcpLoadAsNumber(reg);
            } else {
    ...
    private void visitSetProp(int type, Node node, Node child)
    {
        ...
        child = child.getNext();
        if (type == Token.SETPROP_OP) {
            // stack: ... JObject JObject name -> ... JObject name JObject name
            cfw.add(ByteCode.DUP_X1);
    ...
    private void visitSetElem(int type, Node node, Node child)
    {
        ...
        if (type == Token.SETELEM_OP) {
            if (indexIsNumber) {
                // stack: ... JObject JObject number
                //        -> ... JObject number JObject number
                ...
                // stack: ... JObject JObject indexObject
                //        -> ... JObject indexObject JObject indexObject
                cfw.add(ByteCode.DUP_X1);
                cfw.addALoad(contextLocal);
                addScriptRuntimeInvoke(
                    "getObjectElem",
    ...
    static   int GENERATOR_TERMINATE = -1;
    static   int GENERATOR_START = 0;
    static   int GENERATOR_YIELD_START = 1;
    ...
    private static   int MAX_LOCALS = 1024;
    private int[] locals;
