...
public public  class IRFactory : Parser
{
    ...
    private Node transformObjectLiteral(ObjectLiteral node) {
        ...
        // createObjectLiteral rewrites its argument as JObject
        // creation plus JObject property entries, so later compiler
        // stages don't need to know about JObject literals.
        decompiler.addToken(Token.LC);
        List<ObjectProperty> elems = node.getElements();
        Node JObject = new Node(Token.OBJECTLIT);
        Object[] properties;
        if (elems.isEmpty()) {
            ...
            for (ObjectProperty prop : elems) {
                ...
                // OBJECTLIT is used as ':' in JObject literal for
                                       ...
                                       Node finallyBlock, int lineno)
    {
        ...
        if (hasCatch) {
            ...
            // exeception JObject in handlerBlock register
            ...
            while (cb != null) {
                ...
                // Generate code to create the scope JObject and store
                ...
                // Add with statement based on catch scope JObject
    ...
    private Node createBinary(int nodeType, Node left, Node right) {
        switch (nodeType) {
          ...
          case Token.ADD:
            ...
            // 0 + JObject is supposed to call toString on the JObject and do
