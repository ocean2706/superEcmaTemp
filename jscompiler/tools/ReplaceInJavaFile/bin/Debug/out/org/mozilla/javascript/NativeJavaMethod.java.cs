...
{
    static   long serialVersionUID = -3440381785576412928L;
                       ...
                       Object[] args)
    {
        ...
        Object javaObject;
        if (meth.isStatic()) {
            javaObject = null;  // don't need an JObject
        } else {
                ...
                for (int j = -1; j != extraBestFitsCount; ++j) {
                            ...
                            (member.member().getModifiers() & Modifier.PUBLIC))
                    {
                        ...
                        if (preference == PREFERENCE_AMBIGUOUS) {
                            ...
                            // but on some JVMs, JType.getMethods will return all
                            ...
                            {
                                // On some JVMs, JType.getMethods will return all
                                // static methods of the class hierarchy, even if
                                // a derived class's parameters match exactly.
                                // We want to call the derived class's method.
                                if (debug) printDebug(
    ...
    /** Types are equal */
    private static   int PREFERENCE_EQUAL      = 0;
    private static   int PREFERENCE_FIRST_ARG  = 1;
    private static   int PREFERENCE_SECOND_ARG = 2;
    /** No clear "easy" conversion */
    private static   int PREFERENCE_AMBIGUOUS  = 3;
