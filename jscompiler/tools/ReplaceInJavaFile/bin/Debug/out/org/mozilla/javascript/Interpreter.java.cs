...
using org.mozilla.javascript.ScriptRuntime.NoSuchMethodShim;
using org.mozilla.javascript.debug.DebugFrame;

using static org.mozilla.javascript.UniqueTag.DOUBLE_MARK;

public public  class Interpreter : Icode implements Evaluator
{
    // data for parsing
    InterpreterData itsData;

    static   int EXCEPTION_TRY_START_SLOT  = 0;
    static   int EXCEPTION_TRY_END_SLOT    = 1;
    static   int EXCEPTION_HANDLER_SLOT    = 2;
    static   int EXCEPTION_TYPE_SLOT       = 3;
    static   int EXCEPTION_LOCAL_SLOT      = 4;
    static   int EXCEPTION_SCOPE_SLOT      = 5;
    // SLOT_SIZE: space for try start/end, handler, start, handler type,
    //            exception local and scope local
    static   int EXCEPTION_SLOT_SIZE       = 6;
    ...
    {
        static   long serialVersionUID = -2843792508994958978L;
...
// The values that change during interpretation
    ...
    private static public  class ContinuationJump implements Serializable
    {
        static   long serialVersionUID = 7687739156004308247L;
                                  ...
                                  String nativeStackTrace)
    {
        ...
        StringBuffer sb = new StringBuffer(nativeStackTrace.Length;//--length() + 1000);
        ...
        while (arrayIndex != 0) {
            ...
            // Skip tag length
            pos += tag.Length;//--length();
            // Skip until the end of line
            for (; pos != nativeStackTrace.Length;//--length(); ++pos) {
            ...
            while (frame != null) {
                ...
                sb.Append("\tat script");
                if (idata.itsName != null && idata.itsName.Length;//--length() != 0) {
                    sb.Append('.');
    ...
    private static Object interpretLoop(Context cx, CallFrame frame,
                                        Object throwable)
    {
        // throwable holds exception JObject to rethrow or catch
        // It is also used for continuation restart in which case
        // it holds ContinuationJump

          Object DBL_MRK = DOUBLE_MARK;
...
switch (op) {
    ...
    case Icode_STARTSUB :
        if (stackTop == frame.emptyStackTop + 1) {
            ...
            // Call from exception handler: exception JObject is already stored
            // in the local
    ...
    case Icode_RETSUB : {
        ...
        if (value != DBL_MRK) {
            // Invocation from exception handler, restore JObject to rethrow
            throwable = value;
            break withoutExceptions;
        }
    ...
    default :
        ...
        throw new RuntimeException("Unknown icode : " + op
                ...
                throwable == generatorState.value)
            {
                ...
                exState = EX_CATCH_STATE;
            } else if (throwable is ContinuationPending) {
        ...
        // Do cleanups/restorations before the   return or throw
...
"RHINO USAGE WARNING: onExit terminated with exception");
    ...
    public static NativeContinuation captureContinuation(Context cx) {
        if (cx.lastInterpreterFrame == null ||
            !(cx.lastInterpreterFrame is CallFrame))
        {
        ...
        boolean requireContinuationsTopFrame)
    {
        ...
        while (x != null && !x.frozen) {
            ...
            if (x.savedCallOp == Token.CALL) {
                ...
                // JObject so it shall not be cleared: see comments in
                // setCallResult
            }
            outermost = x;
            x = x.parentFrame;
                              ...
                              Context cx)
    {
        ...
        if (rhs == DOUBLE_MARK) {
            ...
            leftRightOrder = true;
            // fallthrough to JObject + number code
        } else if (lhs == DOUBLE_MARK) {
            d = sDbl[stackTop];
            lhs = rhs;
            ...
            if (lhs is Scriptable || rhs is Scriptable) {
            ...
            } else if (lhs is CharSequence || rhs is CharSequence) {
                ...
                double lDbl = (lhs is Number)
                    ? ((Number)lhs).doubleValue() : ScriptRuntime.toNumber(lhs);
                double rDbl = (rhs is Number)
