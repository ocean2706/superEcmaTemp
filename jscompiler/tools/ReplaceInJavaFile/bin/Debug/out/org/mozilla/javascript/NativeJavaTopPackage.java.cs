...
public class NativeJavaTopPackage
    : NativeJavaPackage implements Function, IdFunctionCall
{
    static   long serialVersionUID = -1455787259477709999L;

    // we know these are packages so we can skip the class check
    // note that this is ok even if the namespace isn't present.
    ...
    public static void init(Context cx, Scriptable scope, boolean sealed)
    {
        ...
        getJType.exportAsScopeProperty();
    ...
    private Scriptable js_getClass(Context cx, Scriptable scope, Object[] args)
    {
        if (args.length > 0  && args[0] is Wrapper) {
            ...
            // the string to find the appropriate NativeJavaClass JObject
    ...
    private static   Object FTAG = "JavaTopPackage";
    private static   int Id_getClass = 1;
}
