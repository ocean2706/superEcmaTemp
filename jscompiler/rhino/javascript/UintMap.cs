...
{
    static readonly long serialVersionUID = 4242698212885848444L;
...
// follows "The Art of Computer Programming" by Donald E. Knuth
    ...
    public void put(int key, int value) {
        ...
        if (ivaluesShift == 0) {
            int N = 1 << power;
            // keys.Lengthcan be N * 2 after clear which set ivaluesShift to 0
            if (keys.Length!= N * 2) {
                int[] tmp = new int[N * 2];
...
// Ensure key index creating one if necessary
        ...
        throws IOException
    {
        ...
        if (count != 0) {
            ...
            out.writebool (hasIntValues);
            out.writebool (hasObjectValues);
        ...
        throws IOException, ClassNotFoundException
    {
        ...
        if (writtenKeyCount != 0) {
            keyCount = 0;
            bool  hasIntValues = in.readbool ();
            bool  hasObjectValues = in.readbool ();
...
// See Knuth etc.
    private static readonly int A = 0x9e3779b9;

    private static readonly int EMPTY = -1;
    private static readonly int DELETED = -2;
