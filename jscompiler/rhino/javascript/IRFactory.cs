 ...
 * @author Norris Boyd
 */
public public class  IRFactory  :  Parser
{
    private static readonly int LOOP_DO_WHILE = 0;
    private static readonly int LOOP_WHILE    = 1;
    private static readonly int LOOP_FOR      = 2;

    private static readonly int ALWAYS_TRUE_bool  = 1;
    private static readonly int ALWAYS_FALSE_bool  = -1;
    ...
    private Node transformParenExpr(ParenthesizedExpression node) {
        ...
        result.putProp(Node.PARENTHESIZED_PROP, bool .TRUE);
        return result;
    }
    ...
    private Node transformReturn(ReturnStatement node) {
        bool  expClosure = bool .TRUE.equals(node.getProp(Node.EXPRESSION_CLOSURE_PROP));
        if (expClosure) {
            decompiler.addName(" ");
        } else {
    ...
    private Node createForIn(int declType, Node loop, Node lhs,
                             Node obj, Node body, bool  isForEach)
    {
        int destructuring = -1;
    ...
    {
        int condStatus = isAlwaysDefinedbool (cond);
        if (condStatus == ALWAYS_TRUE_bool ) {
            return ifTrue;
        } else if (condStatus == ALWAYS_FALSE_bool ) {
            if (ifFalse != null) {
                return ifFalse;
            }
    ...
    private Node createUnary(int nodeType, Node child)
    {
        ...
        switch (nodeType) {
          ...
          case Token.NEG:
            ...
            break;
          case Token.NOT: {
            int status = isAlwaysDefinedbool (child);
            if (status != 0) {
                int type;
                if (status == ALWAYS_TRUE_bool ) {
                    type = Token.FALSE;
                } else {
                    type = Token.TRUE;
                }
    ...
    private Node createBinary(int nodeType, Node left, Node right) {
        switch (nodeType) {
          ...
          case Token.AND: {
            // Since x && y gives x, not false, when bool (x) is false,
            // and y, not bool (y), when bool (x) is true, x && y
            // can only be simplified if x is defined. See bug 309957.

            int leftStatus = isAlwaysDefinedbool (left);
            if (leftStatus == ALWAYS_FALSE_bool ) {
                // if the first one is false, just return it
                return left;
            } else if (leftStatus == ALWAYS_TRUE_bool ) {
          ...
          case Token.OR: {
            // Since x || y gives x, not true, when bool (x) is true,
            // and y, not bool (y), when bool (x) is false, x || y
            // can only be simplified if x is defined. See bug 309957.

            int leftStatus = isAlwaysDefinedbool (left);
            if (leftStatus == ALWAYS_TRUE_bool ) {
                // if the first one is true, just return it
                return left;
            } else if (leftStatus == ALWAYS_FALSE_bool ) {
    ...
    // Check if Node always mean true or false in bool  context
    private static int isAlwaysDefinedbool (Node node) {
        switch (node.getType()) {
          case Token.FALSE:
          case Token.NULL:
            return ALWAYS_FALSE_bool ;
          case Token.TRUE:
            return ALWAYS_TRUE_bool ;
          case Token.NUMBER: {
    ...
    bool  isDestructuring(Node n) {
        return n is DestructuringForm
    ...
    void decompileObjectLiteral(ObjectLiteral node) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            bool  destructuringShorthand =
                    bool .TRUE.equals(prop.getProp(Node.DESTRUCTURING_SHORTHAND));
